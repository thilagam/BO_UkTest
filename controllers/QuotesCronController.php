 ...
 class QuotesCronController extends Zend_Controller_Action
 {
	...
	public function init()
	{
       ...
       $this->closedreason = array(
		...
		$this->_view->fo_path=$this->fo_path='http://ep-test.edit-place.co.uk/FO/';
	}
 
	function getCustomName($type,$name)
	{
		$categories_array = $this->_arrayDb->loadArrayv2($type, 'en');
		return $categories_array[$name];
	}
	public function getLanguageName($lang_value)
    {
        $language_array=$this->_arrayDb->loadArrayv2("EP_LANGUAGES",'en');
	...
	*/
	function quoteChallengeDelayAction()
	{
		...
		$quotescontract_obj = new Ep_Quote_Quotecontract();
		$mail_obj=new Ep_Message_AutoEmails();
			...
			foreach ($lateChallengeQuotes as $quote)
			{
				if($quote['tec_review']=='not_done')
				{
					...
					$receiver_id=$quote['quote_by'];
						$quoteuser=$client_obj->getQuoteUserDetails($receiver_id);
						$mail_param['bo_user_name'] = $quoteuser[0]['first_name']." ".$quoteuser[0]['last_name'];
						$mail_param['quote_title']	= $quote['title'];
						$mail_param['turn_over'] = $quote['final_turnover'];
						$mail_param['client_id'] = $quote['client_id'];
						$mail_param['client_name'] = $quote['company_name'];
						$mail_param['client_logo'] = $this->fo_path."profiles/clients/logos/".$quote['client_id']."/".$quote['client_id']."_global.png?12345";
						$mail_param['challenge_type']='Tech';
						$mail_param['challenge_cron']=true;
						$mail_param['subject']=$quote['title'].' quote '.$mail_param['challenge_type'].' challenge is running late';
						$mail_param['followup']='http://'.$_SERVER['HTTP_HOST'].'/quote-new/sales-final-validation?qaction=briefing&quote_id='.$quote['identifier'];					
					...
					$client_obj=new Ep_Quote_Client();
					/*new tech mission type based  added user*/
					$searchparam['quote_id']=$quote['identifier'];
						$techmissions=$techmission_obj->getTechMissionDetails($searchparam);
						$adduser=array('141127175625894'=>'schateau@edit-place.com',
										'150717153700319'=>'fboucheron@edit-place.com');
						foreach($techmissions as $techdetail)
						{
							$flagdetails=$quotescontract_obj->getTechFlag($techdetail['identifier']);
							if($flagdetails[0]['tech_type_assign']=='edito')
							{
								$challenge_users=$client_obj->getEPContacts('"prodsubmanager","prodmanager"');
							}
							else
							{
								$challenge_users=$client_obj->getEPContacts('"techmanager"');	
								$challenge_users=$challenge_users+$adduser;
							}
						}
						$challenge_users=array_unique($challenge_users);
					//echo 'test'; print_r($challenge_users); exit;
					if(count($challenge_users)>0)
					{
						...
						{
								$mail_param=array();
									$quoteuser=$client_obj->getQuoteUserDetails($user);
									$mail_param['bo_user_name'] = $quoteuser[0]['first_name']." ".$quoteuser[0]['last_name'];
									$mail_param['quote_title']	= $quote['title'];
									$mail_param['turn_over'] = $quote['final_turnover'];
									$mail_param['client_id'] = $quote['client_id'];
									$mail_param['client_name'] = $quote['company_name'];
									$mail_param['challenge_type']='Tech';
									$mail_param['challenge_manager']=true;
									$mail_param['client_logo'] = $this->fo_path.'profiles/clients/logos/'.$quote['client_id'].'/'.$quote['client_id'].'_global.png?12345';
									$mail_param['subject']=$quote['title'].' quote '.$mail_param['challenge_type'].' challenge is running late';
									$mail_param['followup']='http://'.$_SERVER['HTTP_HOST'].'/quote-new/sales-final-validation?qaction=briefing&quote_id='.$quote['identifier'];								
									$this->getNewQuoteEmails($mail_param);
				...
				if($quote['seo_review']=='not_done')
				{
					...
					$client_obj=new Ep_Quote_Client();
					$challenge_users=array();
					$challenge_users=$client_obj->getEPContacts('"seomanager"');
					if(count($challenge_users)>0)
					{
	...
	function relancerEmailAction(){
		...
		$mail_obj=new Ep_Message_AutoEmails();
		$client_obj = new Ep_Quote_Client();
		$contact=new Ep_Quote_ClientContacts();
		...
		if(count($quotes)>0)
					{
							...
							if($vales)
						{
							...
							$mail_param=array();
										...
										$receiver_id=$vales['quote_by'];
										$quoteuser=$client_obj->getQuoteUserDetails($receiver_id);
										$contact_client=$contact->getClientContactsDetails($vales['client_id']);
										$mail_param['contactname']=$contact_client[0]['first_name'];
										$mail_param['contactemail']=$contact_client[0]['email'];
										$mail_param['bo_user_name'] = $quoteuser[0]['first_name']." ".$quoteuser[0]['last_name'];
										$mail_param['latestcommets']	=$vales['latestcommets'];
										$mail_param['quote_title']	= $vales['title'];
										$mail_param['turn_over'] = $vales['turnover'];
										$mail_param['estimate_sign_percentage']=$vales['estimate_sign_percentage'];
										$mail_param['agency_name']=$vales['agency_name'];
										$mail_param['client_id'] = $vales['client_id'];
										$mail_param['client_name'] = $vales['company_name'];
										$mail_param['client_logo'] = $this->fo_path."profiles/clients/logos/".$vales['client_id']."/".$vales['client_id']."_global.png?12345";
										$mail_param['relacnc_email']=TRUE;
										$mail_param['subject']='Send a reminder for '.$vales['company_name'].' quote';
										$mail_param['photo']=$this->_view->fo_path.'profiles/bo/'.$vales['quote_by'].'/logo.jpg';
										$mail_param['downloadlink']='http://'.$_SERVER['HTTP_HOST'].'/quote-new/sales-quotes-list';
										$mail_param['button']='Send a reminder';	
		...
		function closedQuoteEmailAction(){
			...
			//echo 'count'.count($quotes); exit;
			
			if(count($quotes)>0){
				
			  	 foreach($quotes as $quotesloop){
					
						if(($quotesloop['sales_review']=='validated' && $currDate==date('Y-m-d',strtotime($quotesloop['validateddate']. "$days5"))) || ($quotesloop['sales_review']=='closed' && $currDate==date('Y-m-d',strtotime($quotesloop['closedate']. "$days5"))) )
							{
							//Email after 5 days closed/Validated
								//echo 'test5'; exit;
										$mail_param=array();
										
										/*$user_info = $user_obj->getAllUsersDetails($quotesloop['quote_by']);
										...
										$receiver_id=$quotesloop['quote_by'];
										$quoteuser=$client_obj->getQuoteUserDetails($receiver_id);
										$contact_client=$contact->getClientContactsDetails($quotesloop['client_id']);
										$mail_param['contactname']=$contact_client[0]['first_name'];
										$mail_param['contactemail']=$contact_client[0]['email'];
										$mail_param['contactjob']=$contact_client[0]['job_title'];
										$mail_param['bo_user_name'] = $quoteuser[0]['first_name']." ".$quoteuser[0]['last_name'];
										$mail_param['quote_title']	= $quotesloop['title'];
										$mail_param['turn_over'] = $quotesloop['turnover'];
										$mail_param['client_id'] = $quotesloop['client_id'];
										$mail_param['client_name'] = $quotesloop['company_name'];
										$mail_param['client_logo'] = $this->fo_path."profiles/clients/logos/".$quotesloop['client_id']."/".$quotesloop['client_id']."_global.png?12345";
										$mail_param['reminder_5_days']=true;
										$mail_param['subject']=$quotesloop['title'].' 5 days reminder';
										$mail_param['downloadlink']='http://'.$_SERVER['HTTP_HOST'].'/quote-new/download-quote-xls?quote_id='.$quotesloop['identifier'];
										$mail_param['button']='Download your quote';	
										//echo '<pre>'; print_r($mail_param);				
										$this->getNewQuoteEmails($mail_param);
							}
							elseif( ($quotesloop['sales_review']=='validated' && $currDate==date('Y-m-d',strtotime($quotesloop['validateddate']. "$days20"))) || ($quotesloop['sales_review']=='closed' && $currDate==date('Y-m-d',strtotime($quotesloop['closedate']. "$days20"))) )
							{
								//Email after 20 days closed/Validated
										...
										$mail_param['client_name'] = $quotesloop['company_name'];
										$mail_param['client_logo'] = $this->fo_path."profiles/clients/logos/".$quotesloop['client_id']."/".$quotesloop['client_id']."_global.png?12345";
										$mail_param['reminder_20_days']=true;
										$mail_param['subject']=$quotesloop['title'].' expired';
										$mail_param['downloadlink']='http://'.$_SERVER['HTTP_HOST'].'/quote-new/sales-final-validation?qaction=briefing&quote_id='.$quotesloop['identifier'];	
										$mail_param['button']='Refresh your quote';				
										$this->getNewQuoteEmails($mail_param);
							}
							elseif( ($quotesloop['sales_review']=='validated' && $currDate==date('Y-m-d',strtotime($quotesloop['validateddate']. "$days30"))) ||  ($quotesloop['sales_review']=='closed' && $currDate==date('Y-m-d',strtotime($quotesloop['closedate']. "$days30"))) )
							{
								//Email after 30 days closed/Validated
										...
										$mail_param['client_name'] = $quotesloop['company_name'];
										$mail_param['client_logo'] = $this->fo_path."profiles/clients/logos/".$quotesloop['client_id']."/".$quotesloop['client_id']."_global.png?12345";
										$mail_param['reminder_30_days']=true;
										$mail_param['subject']=$quotesloop['title'].' to close';
										$mail_param['downloadlink']='http://'.$_SERVER['HTTP_HOST'].'/quote-new/sales-quotes-list?quote_id='.$quotesloop['identifier'].'#quote-'.$quotesloop['identifier'];
										$mail_param['button']='Clean Pending Feedback Section';	
										$this->getNewQuoteEmails($mail_param);
							}
	...
	function getNewQuoteEmails($email)
	{
		                ...
		                <tr>
		                    <td align="center" valign="top" id="bodyCell" style="mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;height: 100%;margin: 0;padding: 10px;width: 100%;border-top: 0;">
								...
								<table border="0" cellpadding="0" cellspacing="0" width="100%" class="templateContainer" style="border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;border: 0;max-width: 600px !important;">
									<tr>
										<td valign="top" id="templatePreheader" style="mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;background-color: #f5f5f5;border-top: 0;border-bottom: 0;padding-top: 9px;padding-bottom: 9px;"><table border="0" cellpadding="0" cellspacing="0" width="100%" class="mcnCaptionBlock" style="border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		...
		<table border="0" cellpadding="0" cellspacing="0" class="mcnCaptionRightContentOuter" width="100%" style="border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		    <tbody><tr>
		        <td valign="top" class="mcnCaptionRightContentInner" style="padding: 0 9px;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		            <table align="left" border="0" cellpadding="0" cellspacing="0" class="mcnCaptionRightImageContentContainer" style="border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;height:72px;width:150px;">
		                <tbody><tr>
		                    <td class="mcnCaptionRightImageContent" valign="top" style="mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		                        <img alt="" src="http://admin-ep.edit-place.com/BO/theme/gebo/css/images/workplace.png" width="102" style="max-width: 102px;border: 0;height: auto;outline: none;text-decoration: none;-ms-interpolation-mode: bicubic;vertical-align: bottom;" class="mcnImage">
		                    </td>
		                </tr>
		            </tbody></table>
		            <table class="mcnCaptionRightTextContentContainer" align="right" border="0" cellpadding="0" cellspacing="0" width="396" style="border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		                <tbody><tr>
		                    <td valign="top" class="mcnTextContent" style="font-size: 20px;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;word-break: break-word;color: #454545;font-family: Helvetica;line-height: 150%;text-align: left;">
		                    </td>
		                </tr>
		            </tbody></table>
		        </td>
		    ...
		    <tbody><tr>
		        <td valign="top" class="mcnCaptionRightContentInner" style="padding: 0 9px;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		            <table align="left" border="0" cellpadding="0" cellspacing="0" class="mcnCaptionRightImageContentContainer" style="border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		                <tbody><tr>
		                    <td class="mcnCaptionRightImageContent" valign="top" style="mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		                        <img alt="" src='.$email['client_logo'].' width="72" height="72" style="max-width: 225px;border: 0;height: auto;outline: none;text-decoration: none;-ms-interpolation-mode: bicubic;vertical-align: bottom;" class="mcnImage">
		                    </td>
		                </tr>
		            </tbody></table>
		            <table class="mcnCaptionRightTextContentContainer" align="right" border="0" cellpadding="0" cellspacing="0" width="396" style="border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		                <tbody><tr>
		                    <td valign="top" class="mcnTextContent" style="line-height: 150%;text-align: left;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;word-break: break-word;color: #454545;font-family: Helvetica;font-size: 16px;">
		                        <span style="font-size:14px"> '.$email['client_name'].'.</span><br>';
		                        if ($email['challenge_cron'] || $email['challenge_manager'] || $email['reminder_5_days'] || $email['reminder_20_days'] || $email['reminder_30_days'])
		                        {	
		                       		$email_content.= '<span style="font-size:22px">Quote total HT: '.$email['turn_over'].' &euro;</span><br>
		                        <span style="font-size:14px">Signed &#10003;                 <br>';
		                    	}
		                    	elseif($email['relacnc_email'])
		                    	{
		                    		$email_content.= '<span style="font-size:22px">Quote turnover : '.$email['turn_over'].' &euro;</span><br>
			                        <span style="font-size:14px">Probability of signature : <strong>'.$email['estimate_sign_percentage'].'%</strong><br>';
			                        if($email['agency_name'])
			                            $email_content.= 'Competition with : '.$email['agency_name'];
			                     $email_content.= '</span>';
		                    	}
		                      $email_content.=' </span>
		                    </td>
		...
		if($email['reminder_5_days'] || $email['reminder_20_days'] )
		{
		 $email_content.='<table border="0" cellpadding="0" cellspacing="0" width="100%" class="mcnDividerBlock" style="min-width: 100%;border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;table-layout: fixed !important;">
...
-->
			 ...
			 $email_content.='<table border="0" cellpadding="0" cellspacing="0" class="mcnCaptionRightContentOuter" width="100%" style="border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
			    <tbody>
			    	<tr>
				        <td valign="top" class="mcnCaptionRightContentInner" style="padding: 0 9px;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
				            <table class="mcnCaptionRightTextContentContainer" border="0" cellpadding="0" cellspacing="0"style="border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
				                <tbody>
				                	<tr>
					                    <td style="padding-top: 9px; padding-right: 18px; padding-bottom: 9px; padding-left: 18px; word-break: break-word; color: #656565; font-family: Helvetica; font-size: 12px; line-height: 150%; text-align: left;" valign="top">
						                   	<p style="line-height: 20.8px; margin: 10px  0; padding: 0; color: #656565; font-family: Helvetica; font-size: 12px; text-align: left;"><span style="font-size: 14px;">Customer contact details</span><br><br><strong><span style="font-size: 14px;">'.$email['contactname'].'</span></strong><br><span style="line-height: 20.8px;">'.$email['contactjob'].'&nbsp;</span></p>
						                   	<p style="line-height: 20.8px; margin: 10px  0; padding: 0; color: #656565; font-family: Helvetica; font-size: 12px; text-align: left;"><span><a style="color: #00a2ff; font-weight: normal; text-decoration: underline;" href="./#NOP" onclick="return false"></a></span></p>
						             		<p style="line-height: 20.8px; margin: 10px  0; padding: 0; color: #656565; font-family: Helvetica; font-size: 12px; text-align: left;"><span><a href="mailto:'.$email['contactemail'].'" style="color: #00a2ff; font-weight: normal; text-decoration: underline;" onclick=."return rcmail.command(compose,'.$email['contactemail'].',this)">'.$email['contactemail'].'</a></span></p>
										</td>
					                </tr>
					            </tbody>
					        </table>
		...
		if ($email['sales_user'] || $email['relacnc_email'] || $email['reminder_30_days'])
		{
		$email_content.='<table border="0" cellpadding="0" cellspacing="0" width="100%" class="mcnDividerBlock" style="min-width: 100%;border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;table-layout: fixed !important;">
		    <tbody class="mcnDividerBlockOuter">
		        <tr>
		                ...
		                <td valign="top" style="padding: 9px;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;" class="mcnImageBlockInner">
		                    <table align="left" width="100%" border="0" cellpadding="0" cellspacing="0" class="mcnImageContentContainer" style="min-width: 100%;border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		                        <tbody><tr>
		                            ...
		                            if($email['sales_user'])
		                            {
		                              $email_content.='<img align="center" alt="" src="'.$this->fo_path.'/profiles/bo/'.$email['sales_user'].'/logo.jpg" width="72" style="max-width: 72px;padding-bottom: 0;display: inline !important;vertical-align: bottom;border: 0;height: auto;outline: none;text-decoration: none;-ms-interpolation-mode: bicubic;" class="mcnImage">';
		                            }
		                            $email_content.='</td>
		                        </tr>
		                    </tbody></table>
		...
		$email_content.='<table border="0" cellpadding="0" cellspacing="0" width="100%" class="mcnTextBlock" style="min-width: 100%;border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		        ...
		        <tr>
		            <td valign="top" class="mcnTextBlockInner" style="mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		                <table align="left" border="0" cellpadding="0" cellspacing="0" width="100%" style="min-width: 100%;border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;" class="mcnTextContentContainer">
		                    <tbody><tr>
		                        <td valign="top" class="mcnTextContent" style="padding-top: 9px;padding-right: 18px;padding-bottom: 9px;padding-left: 18px;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;word-break: break-word;color: #454545;font-family: Helvetica;font-size: 16px;line-height: 150%;text-align: left;">';
		                            if ($email['challenge_cron'])
		                            {
		                             $email_content.=' <div style="text-align: center;">
		                              <span style="font-size:14px">Dear '.$email['bo_user_name'].',<br>
		                                    '.$email['challenge_type'].' team has not challenged your quote '.$email['quote_title'].' yet and it is running late.</span><br>
		                                         &nbsp;
		                                          <span style="font-size:14px">A reminder has been sent to the team. </span>
		                                  </div>';
		                              }
		                            elseif($email['challenge_manager'])
		                            {
		                              $email_content.='<div style="text-align: center;">
		                              <span style="font-size:14px">Dear '.$email['bo_user_name'].',<br>
		                                    You have not challenged the '.$email['quote_title'].' yet and it is running late.</span><br>
		                                         &nbsp;
		                                          <span style="font-size:14px">Please challenge it now. . </span>
		                                  </div>';
		                            }
		                            elseif($email['reminder_5_days'])
		                            {
		                            	 $email_content.='<div style="text-align: center;">
		                              <span style="font-size:14px">Dear '.$email['bo_user_name'].',<br>
		                                    You have sent '.$email['quote_title'].' to '.$email['contactname'].' five days ago.</span><br>
		                                         &nbsp;
		                                          <span style="font-size:14px"> <a href="mailto:'.$email['contactemail'].'" style="color: #00a2ff; font-weight: normal; text-decoration: underline;" onclick=."return rcmail.command(compose,'.$email['contactemail'].',this)">Click here for sending a mail</a></span>
		                                  </div>';
		                            }
		                            elseif($email['reminder_20_days'])
		                            {
		                            	 $email_content.='<div style="text-align: center;">
		                              <span style="font-size:14px">Dear '.$email['bo_user_name'].',<br>
		                                    Any client answer to '.$email['quote_title'].' sent 20 days ago.<br>
		                                       Unfortunatelly your quote has expired and can no longer be used.<br><br></span>
		                                          <span style="font-size:14px">See the Probability of signature and please send '.$email['contactname'].' a reminder.<br>
		                                          Try something different or shut your quote down ! 
		                                          </span>
		                                  </div>';
		                            }
		                            elseif($email['reminder_30_days'])
		                            {
		                            	 $email_content.='<div style="text-align: center;">
		                              <span style="font-size:14px">Dear '.$email['bo_user_name'].',<br>
		                                 <br></span>
		                                          <span style="font-size:14px">The quote '.$email['quote_title'].' quote has been sent to the client '.$email['client_name'].' 30 days ago.<br>
		                                         It`s been a while. Please verify your quote and clean Pending feedback section !
		                                          </span>
		                                  </div>';
		                            }
		                            elseif($email['relacnc_email'])
		                            {
										 $email_content.='<div style="text-align: center;">
		                              <span style="font-size:14px">Dear '.$email['bo_user_name'].',<br>
		                                         You have planned to send '.$email['contactname'].' a reminder about '.$email['quote_title'].'.<br>
		                                         Please, send a reminder <br> <br> </span>
		                                          <span style="font-size:14px">For your information, see your last comment :<br>
		                                         	<img align="center" alt="" src="'.$email['photo'].'" width="146" style="max-width: 50px;padding-bottom: 0;display: inline !important;vertical-align: center;border: 0;height: auto;outline: none;text-decoration: none;-ms-interpolation-mode: bicubic;" class="mcnImage">'.$email['latestcommets'].'
		                                          </span>
		                                  </div>';
		                            }
		                        $email_content.='</td>
		...
		if ($email['followup'] || $email['downloadlink'])
		{
		$email_content.='<table border="0" cellpadding="0" cellspacing="0" width="100%" class="mcnButtonBlock" style="min-width: 100%;border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		    <tbody class="mcnButtonBlockOuter">
		        <tr>
		            <td style="padding-top: 0;padding-right: 18px;padding-bottom: 18px;padding-left: 18px;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;" valign="top" align="center" class="mcnButtonBlockInner">
		                <table border="0" cellpadding="0" cellspacing="0" width="100%" class="mcnButtonContentContainer" style="border-collapse: separate !important;border-radius: 3px;background-color: #448AFF;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		                    <tbody>
		                        <tr>
		                            ...
		                            if($email['downloadlink'] && $email['relacnc_email'])
		                            {
		                               $email_content.= '<a href="mailto:'.$email['contactemail'].'" class="mcnButton " title="Answer" style="font-weight: bold;letter-spacing: normal;line-height: 100%;text-align: center;text-decoration: none;color: #FFFFFF;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;display: block;" onclick="return rcmail.command(compose,'.$email['contactemail'].',this)">'.$email['button'].'</a>';
		                            }
		                            elseif($email['downloadlink'] && ($email['reminder_30_days'] || $email['reminder_20_days'] || $email['reminder_5_days'] ))
		                            {
		                            	$email_content.= '<a class="mcnButton " title="Answer" href="'.$email['downloadlink'].'" target="_blank" style="font-weight: bold;letter-spacing: normal;line-height: 100%;text-align: center;text-decoration: none;color: #FFFFFF;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;display: block;">'.$email['button'].'</a>';
		                            }
		                           else
		                           {
		                           	$email_content.= '<a class="mcnButton " title="Answer" href="'.$email['followup'].'" target="_blank" style="font-weight: bold;letter-spacing: normal;line-height: 100%;text-align: center;text-decoration: none;color: #FFFFFF;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;display: block;"> View the quote</a>';
		                           }
		                           $email_content.= '</td>
		                        </tr>
		                    </tbody>
		    ...
		    <tbody class="mcnTextBlockOuter">
		        <tr>
		            <td valign="top" class="mcnTextBlockInner" style="mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		                <table align="left" border="0" cellpadding="0" cellspacing="0" width="100%" style="min-width: 100%;border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;" class="mcnTextContentContainer">
		                    <tbody><tr>
		                        <td valign="top" class="mcnTextContent" style="padding: 9px 18px;font-size: 12px;line-height: 150%;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;word-break: break-word;color: #454545;font-family: Helvetica;text-align: left;">
		                            <div style="text-align: left; font-size:12"><em>Best,<br>
										Edit-place Team<br>
										www.edit-place.com</div><em>
		                        </td>
		                    </tr>
		                </tbody></table>
		            </td>
									...
									<tr>
										<td valign="top" id="templateFooter" style="mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;background-color: #FAFAFA;border-top: 2px solid #EAEAEA;border-bottom: 0;padding-top: 9px;padding-bottom: 9px;"><table border="0" cellpadding="0" cellspacing="0" width="100%" class="mcnImageBlock" style="min-width: 100%;border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		    <tbody class="mcnImageBlockOuter">
		            <tr>
		                <td valign="top" style="padding: 9px;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;" class="mcnImageBlockInner">
		                    <table align="left" width="100%" border="0" cellpadding="0" cellspacing="0" class="mcnImageContentContainer" style="min-width: 100%;border-collapse: collapse;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		                        <tbody><tr>
		                            <td class="mcnImageContent" valign="top" style="padding-right: 9px;padding-left: 9px;padding-top: 0;padding-bottom: 0;text-align: center;mso-line-height-rule: exactly;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;">
		                                        <img align="center" alt="" src="http://admin-ep.edit-place.com/BO/theme/gebo/css/images/edit-place.png" width="146" style="max-width: 146px;padding-bottom: 0;display: inline !important;vertical-align: bottom;border: 0;height: auto;outline: none;text-decoration: none;-ms-interpolation-mode: bicubic;" class="mcnImage">
		                            </td>
		                        </tr>
		                    </tbody></table>
		                </td>
