...
class ContractmissionController extends Ep_Controller_Action
{
	...
	function getSplitMonthAction()
	{
		if($this->_request->isPost() && $_SERVER['HTTP_X_REQUESTED_WITH']=='XMLHttpRequest')
		{
			...
			else */
			{
				...
				$tech_missions = $tech_obj->getTechMissionDetails(array('quote_id'=>$quote_id,'include_final'=>'yes','before_prod_all'=>'Before'));
				if($tech_missions)
				{
					$i = 0;
					for($i=0;$i<count($tech_missions);$i++)
					{
						if(count($alldays)>0 && $tech_missions[$i]['before_prod']=='After'){
						$max=max($alldays);
	...
	function saveContractAction()
	{
        ...
        {  
			...
			$quote_obj = new Ep_Quote_Quotes();
			if($params['quote_id'] && ( $params['contract_name']=='' || $params['expected_launch_date']=="" || $params['expected_end_date']==""	) )
			{
				$this->_redirect("/contractmission/create-contract?quote_id=".$params['quote_id']);
				return false;
			}
			if($params['quote_id'] )
			{
...
//				echo "<pre>";print_r($save);exit;
					...
					if($params['validate']=="validate")
					{
						...
						if($tech_count)
						{
							...
							foreach($tech_missions as $eachTechMission)
							{
								...
								{
									$savecontractMission['contract_id'] = $params['contract_id'];
									$savecontractMission['type'] ='tech';
									$savecontractMission['type_id'] = $eachTechMission['identifier'];
									$savecontractMission['cm_status'] = 'validated';
									$savecontractMission['assigned_to'] = $quotesDetailsassigned[0]['quote_by'];
									$savecontractMission['assigned_by'] = $this->adminLogin->userId;
									$savecontractMission['updated_by'] = $this->adminLogin->userId;
									$savecontractMission['updated_at'] = date('Y-m-d H:i:s');
									$savecontractMission['validated_by'] = $quotesDetailsassigned[0]['quote_by'];
									$savecontractMission['validated_at'] = date('Y-m-d H:i:s');

									$contract_mission_id = $res = $quote_contract->insertContractMission($savecontractMission);

									$log_obj=new Ep_Quote_QuotesLog();	
									if(strtotime(date('Y-m-d'))>strtotime($save['expected_end_date']))
									{
										$actionmessage = $log_obj->getActionSentence(13);
										$no_of_seconds = strtotime(date('Y-m-d H:i:s')) - strtotime($save['expected_end_date']);
									...
									$log_array['user_id'] = $this->adminLogin->userId;
									$log_array['contract_id'] = $contractid ;
									$log_array['mission_id'] = $eachTechMission['identifier'];
									...
									if($eachTechMission['oneshot']=='no')
									{
										$tempo_length=$eachTechMission['tempo_length'];
										$tempo_length_option=$eachTechMission['tempo_length_option'];
										if($tempo_length_option=='days')
										{
											 $tempo_callenth=$tempo_length;
										}
										elseif($tempo_length_option=='week')	
										{
											$tempo_callenth=$tempo_length*7;
										}
										elseif($tempo_length_option=='month')	
										{
											$tempo_callenth=$tempo_length*30;
										}
										elseif($tempo_length_option=='year')
										{
											$tempo_callenth=$tempo_length*365;
										}
										$evalue=($eachTechMission['volume']/$eachTechMission['volume_max']);
										$taskcreate=ceil($evalue);
										$taskturnover=0;
										$taskcostover=0;
										for ($i=1; $i<=$taskcreate ; $i++) 
										{ 
											$task_data = array();
											$task_data['task_title'] = $eachTechMission['title'].' '.$i;
											$task_data['volume'] =$eachTechMission['volume_max'];
											$task_data['margin_percentage'] = $eachTechMission['margin_percentage'];
											$task_data['mission_type'] = 'tech';
											$task_data['created_by'] = $this->adminLogin->userId;
											
												if($taskcreate==$i)
												{
													$task_data['turnover'] = $eachTechMission['turnover']-$taskturnover;
													$task_data['cost'] = ($eachTechMission['cost']*$eachTechMission['volume'])-($taskcostover);
													
												}
												else
												{
													$task_data['turnover'] = round($eachTechMission['turnover']/$taskcreate);
													$task_data['cost'] = round(($eachTechMission['cost']*$eachTechMission['volume'])/$taskcreate);
													$taskturnover +=$task_data['turnover'];
													$taskcostover +=$task_data['cost'];
												}

												$no_days=($i-1)*$tempo_callenth;

											$task_data['created_at'] =date('Y-m-d H:i:s',strtotime(date('Y-m-d H:i:s')."+ $no_days days")) ;
											$task_data['m_status'] ='validated';
											$task_data['validated_by'] =$this->adminLogin->userId;
											$task_data['validated_at'] =date('Y-m-d H:i:s');
											$task_data['contract_mission_id'] =$contract_mission_id;
											$task_obj->insertTask($task_data);
									...
									else
									{
										$taskcreate=1;
										$task_data = array();
										$task_data['task_title'] = $eachTechMission['title'];
										$task_data['volume'] =$eachTechMission['volume'];
										$task_data['turnover'] = ($eachTechMission['turnover']/$taskcreate);
										$task_data['margin_percentage'] = $eachTechMission['margin_percentage'];
										$task_data['cost'] = ($eachTechMission['cost']*$eachTechMission['volume'])/$taskcreate;
										$task_data['mission_type'] = 'tech';
										$task_data['created_by'] = $this->adminLogin->userId;
			...
			if($contract)
			{
				...
				foreach($invoices as $invoice):
					...
					$invoices[$i]['title'] = $this->product_array[$invoice['product']]." ".$this->producttype_array[$invoice['product_type']]." in ".$this->getCustomName("EP_LANGUAGES",$invoice['language_source']);
                                        ...
                                        if($invoice['product_type']=="autre" || $invoice['product_type']=="wordings")
                                        {
		...
		if($this->_request->isPost() && $_SERVER['HTTP_X_REQUESTED_WITH']=='XMLHttpRequest')
		{
			...
			foreach($documents_path as $k=>$file)
			{
				...
				if(file_exists($this->contract_documents_path.$documents_path[$k]) && !is_dir($this->contract_documents_path.$documents_path[$k]))
				{
					...
					$files .= '<tr><td width="30%">'.$file_name.'</td><td width="35%">'.substr($ofilename['filename'],0,-3).".".$ofilename['extension'].'</td><td width="20%">'.formatSizeUnits(filesize($file_path)).'</td><td>Contract</td><td align="center" width="15%"><a href="/contractmission/download-document?type=contract&contract_id='.$identifier.'&index='.$k.'"><i style="margin-right:5px" class="splashy-download"></i></a><span class="delete" rel="'.$k.'_'.$identifier.'" id="'.$k.'_'.$identifier.'"> <i class="icon-adt_trash"></i></span></td></tr>';
	...
	function assignMissionAction()
	{
		...
		if($params['contract_id'] && $this->_view->loginuserId):
			...
			if($contract_details)
			{
				...
				if($params['type']=='tech' && ($this->_view->user_type=='techmanager' || $this->_view->user_type=='superadmin' || $this->_view->user_type=='prodmanager'))
				{
					$searchParameters = array();
					$searchParameters['quote_id']=$quote_id;
					$searchParameters['misson_user_type']='sales';
					...
					if(($index < $this->_view->total_tech_mission_count) || $params['cmid'])
					{
						...
						$techtypeFlag=$quote_contract->getTechFlag($techMissionDetails[$index]['identifier']);
							if($techtypeFlag[0]['tech_type_assign']!='sales')
							{
								$techMissionDetails[$index]['techfreeMission']=true;
							}
							else
							{
								$techMissionDetails[$index]['techfreeMission']=false;
							}
						...
						$this->_view->type = 'tech';
							...
							if(count($techtypeFlag)>0 && $techtypeFlag[0]['tech_type_assign'])
							{
								//echo $techtypeFlag[0]['tech_type_assign']; exit;
								if($techtypeFlag[0]['tech_type_assign']=='superadmin')
								{
									$superadmin_user = $quote_contract->getmissionuser("'superadmin','techmanager','techuser'");
									$this->_view->users=$superadmin_user;
									$superadmincount++;
								}
								elseif($techtypeFlag[0]['tech_type_assign']=='edito')
								{
									$prodmanager_user = $quote_contract->getmissionuser("'prodmanager','prodsubmanager','multilingue'");
									$this->_view->users = $prodmanager_user;
									$Editocount++;
								}
								elseif($techtypeFlag[0]['tech_type_assign']=='integration')
								{									
									$techmanagers_user = $quote_contract->getmissionuser("'techmanager','techuser'");
									$this->_view->users =$techmanagers_user;
									$integrationcount++;
								}
								elseif($techtypeFlag[0]['tech_type_assign']=='sales')
								{									
									$salesmanagers_user = $quote_contract->getmissionuser("'salesmanager','salesuser','superadmin'");
									$this->_view->users =$salesmanagers_user;
									$salescount++;
								}
							}							
							else
						...
						$totalcountmissionsassigned = $quote_contract->getCountContractMission($params['contract_id'],'tech');
							if($this->_view->user_type=='superadmin')
							{
								$techCount=$superadmincount+$integrationcount+$Editocount+$salescount;
								$remaining_missions = $techCount;
							}
							elseif($this->_view->user_type=='prodmanager')
							{
								//echo $Editocount;
								$remaining_missions =count($missionDetails)+$Editocount;
							}
							elseif($this->_view->user_type=='techmanager')
							{
								$remaining_missions =$integrationcount;
							}
							else
							{
						...
						if($quote_details[0]['documents_path'])
						{
							...
							foreach($exploded_file_paths as $row)
							{
								...
								if(file_exists($file_path) && !is_dir($file_path))
								{
									...
									$quotefiles .= '<tr><td width="30%">'.$fname.'</td><td width="35%">'.substr($ofilename['filename'],0,-3).".".$ofilename['extension'].'</td><td width="20%">'.formatSizeUnits(filesize($file_path)).'</td><td>Sales</td><td align="center" width="15%"><a href="/quote/download-document?type=quote&quote_id='.$quote_details[0]['identifier'].'&index='.$k.'"><i style="margin-right:5px" class="splashy-download"></i></a><span class="deletequote" rel="'.$k.'_'.$quote_details[0]['identifier'].'" id="'.$k.'_'.$quote_details[0]['identifier'].'"> <i class="icon-adt_trash"></i></span><td></tr>';
								}
								$k++;
							}
							...
							foreach($exploded_file_paths as $row)
							{
								...
								if(file_exists($file_path) && !is_dir($file_path))
								{
									...
									if($fname=="")
										$fname = basename($row);
									$ofilename = pathinfo($file_path);
									$files .= '<tr><td width="30%">'.$fname.'</td><td width="35%">'.substr($ofilename['filename'],0,-3).".".$ofilename['extension'].'</td><td width="20%">'.formatSizeUnits(filesize($file_path)).'</td><td>Staff</td><td align="center" width="15%"><a href="/quote/download-document?type=staff_mission&mission_id='.$staffMissionDetails[$index]['staff_missionId'].'&index='.$k.'"><i style="margin-right:5px" class="splashy-download"></i></a><span class="deletestaff" rel="'.$k.'_'.$staffMissionDetails[$index]['staff_missionId'].'" id="'.$k.'_'.$staffMissionDetails[$index]['staff_missionId'].'"> <i class="icon-adt_trash"></i></span><td></tr>';
								}
								$k++;
							}
						}
				...
				elseif($params['type']=='seo' && ($this->_view->user_type=='seomanager' || $this->_view->user_type=='superadmin'))
				{
					...
					if(($index < $this->_view->total_seo_mission_count) || $params['cmid'])
					{
						...
						$seoMissionDetails[$index]['Maxcost'] = $seoMissionDetails[$index]['internal_cost'];
							...
							foreach($exploded_file_paths as $row)
							{
								...
								if(file_exists($file_path) && !is_dir($file_path))
								{
									...
									if($fname=="")
										$fname = basename($row);
									$ofilename = pathinfo($file_path);
									$files .= '<tr><td width="30%">'.$fname.'</td><td width="35%">'.substr($ofilename['filename'],0,-3).".".$ofilename['extension'].'</td><td width="20%">'.formatSizeUnits(filesize($file_path)).'</td><td>SEO</td><td align="center" width="15%"><a href="/quote/download-document?type=seo_mission&mission_id='.$seoMissionDetails[$index]['identifier'].'&index='.$k.'"><i style="margin-right:5px" class="splashy-download"></i></a><span class="delete" rel="'.$k.'_'.$seoMissionDetails[$index]['identifier'].'" id="'.$k.'_'.$seoMissionDetails[$index]['identifier'].'"> <i class="icon-adt_trash"></i></span><td></tr>';
								}
								$k++;
							}
							$this->_view->files = $files;
        ...
        {  
			...
			$params['writing'] = str_replace(",",".",$params['writing']);
				if($params['contractmissionid']):
					...
					$save['updated_by'] = $this->_view->loginuserId;
					$save['updated_at'] = date('Y-m-d H:i:s');
					if($params['freelance_mission']=='yes')
						{
						$save['external_mission'] = $params['freelance_mission'];
						$save['freelance_name'] = $params['freelacename'];
						$save['freelance_cost'] = $params['freelacecost'];
						}
					if($params['missiontype']=='prod')
					{
						//$save['min_cost'] = $params['min_cost'];
					...
					elseif($params['missiontype']=='tech')
					{
						...
						//echo "<pre>"; print_r($techdetails); exit;
							if(count($techdetails)>0 && $techdetails[0]['is_new_quote']=='1')
							{
								if($contract_mission_id)
								{
									$task_obj = new Ep_Quote_Task();
									if($techdetails[0]['oneshot']=='no')
									{
										$tempo_length=$techdetails[0]['tempo_length'];
										$tempo_length_option=$techdetails[0]['tempo_length_option'];
										if($tempo_length_option=='days')
										{
											 $tempo_callenth=$tempo_length;
										}
										...
										$value=($techdetails[0]['volume']/$techdetails[0]['volume_max']);
										$taskcreate=ceil($value);
										$taskturnover=0;
										$taskcostover=0;
										for ($i=1; $i<=$taskcreate ; $i++) 
										{ 
											$task_data = array();
											$task_data['task_title'] = $techdetails[0]['title'].' '.$i;
											$task_data['volume'] = $techdetails[0]['volume_max'];
											$task_data['margin_percentage'] = $techdetails[0]['margin_percentage'];
											$task_data['mission_type'] = 'tech';
											$task_data['created_by'] = $this->adminLogin->userId;
											
											if($params['freelance_mission']=='yes')
											{
												$cost=$save['freelance_cost'];
											}
											else
											{
												$cost=$techdetails[0]['cost']*$techdetails[0]['volume'];
											}
											
										
												if($taskcreate==$i)
												{
													$task_data['turnover'] = $techdetails[0]['turnover']-$taskturnover;
													$task_data['cost'] = ($cost)-($taskcostover);
													
												}
												else
												{
													$task_data['turnover'] = round($techdetails[0]['turnover']/$taskcreate);
													$task_data['cost'] = round(($cost)/$taskcreate);
													$taskturnover +=$task_data['turnover'];
													$taskcostover +=$task_data['cost'];
												}
									...
									else
									{
										...
										$task_data = array();
										$task_data['task_title'] = $techdetails[0]['title'];
										$task_data['volume'] = $techdetails[0]['volume'];
										$task_data['turnover'] = ($techdetails[0]['turnover']/$taskcreate);
										$task_data['margin_percentage'] = $techdetails[0]['margin_percentage'];
										$task_data['cost'] = ($cost)/$taskcreate;
										$task_data['mission_type'] = 'tech';
										$task_data['created_by'] = $this->adminLogin->userId;
										$task_data['created_at'] = date('Y-m-d H:i:s');
										$task_data['contract_mission_id'] =$contract_mission_id;
	...
	{
		$prod_seo_tech = array();
		//echo 'test '.$this->_view->user_type. ' '.$this->_view->userId ; 
		// prod missions
		if($this->_view->user_type=='prodsubmanager' || $this->_view->user_type=='multilingue')
		{
			...
			//echo '<pre>'; print_r($assignedMissions); exit;
		...
		if($this->_view->user_type=='techuser' || $this->_view->user_type=='prodsubmanager' || $this->_view->user_type=='multilingue' || $this->_view->user_type=='salesuser' || $this->_view->user_type=='salesmanager')
		{
			$search['assigned_to'] = $this->_view->userId;
			$contract_obj = new Ep_Quote_Quotecontract();
	...
	function missionslistManagers($search=array(),$lang="")
	{
		...
		$prodIndex = $seoIndex = $techIndex = 0;
		if($this->_view->user_type=='techmanager' || $this->_view->user_type=='prodmanager')
		{
			$contract_missions = $con_mis_obj->getContractTechMissions($search);
			...
			foreach($contracts as $row)
			{
				...
				foreach($techMissionDetails as $tech_row)
				{
					...
					{
						/*checking tech flag type */
						$display=true;					
						$techtypeFlag=$con_mis_obj->getTechFlag($tech_row['identifier']);
						if(count($techtypeFlag)>0)
						{
							//echo $techtypeFlag[0]['tech_type_assign']."<br>";
							if($techtypeFlag[0]['tech_type_assign']!='integration' && $this->_view->user_type=='techmanager')
							{
								$display=false;
							}
							else if($techtypeFlag[0]['tech_type_assign']!='edito' && $this->_view->user_type=='prodmanager')
							{
								$display=false;
							}
						}	
	...
	function formatMission($row)
	{
		...
		else
		{
			...
			$array['destination_language'] = "-";
                ...
                if($row['product_type']=="autre" || $row['product_type']=="wordings")
                {
		...
		foreach($documents_path as $k=>$file)
		{
			...
			if(file_exists($this->contract_documents_path.$documents_path[$k]) && !is_dir($this->contract_documents_path.$documents_path[$k]))
			{
				...
				if($documents_name[$k])
					...
					$file_name=basename($file);
				$ofilename = pathinfo($file_path);
				$files .= '<tr><td width="30%">'.$file_name.'</td><td width="35%">'.substr($ofilename['filename'],0,-3).".".$ofilename['extension'].'</td><td width="20%">'.formatSizeUnits(filesize($file_path)).'</td><td>Contract</td><td align="center" width="15%"><a href="/contractmission/download-document?type=contract&contract_id='.$contract['contract_id'].'&index='.$k.'"><i style="margin-right:5px" class="splashy-download"></i></a><span class="delete" rel="'.$k.'_'.$contract['contract_id'].'" id="'.$k.'_'.$contract['contract_id'].'"> <i class="icon-adt_trash"></i></span></td></tr>';
		...
		foreach($exploded_file_paths as $row)
		{
			...
			if(file_exists($file_path) && !is_dir($file_path))
			{
				...
				if($fname=="")
					$fname = basename($row);
				$ofilename = pathinfo($file_path);
				$files .= '<tr><td width="30%">'.$fname.'</td><td width="35%">'.substr($ofilename['filename'],0,-3).".".$ofilename['extension'].'</td><td width="20%">'.formatSizeUnits(filesize($file_path)).'</td><td>Tech</td><td align="center" width="15%"><a href="/quote/download-document?type=tech_mission&mission_id='.$mission['id'].'&index='.$k.'"><i style="margin-right:5px" class="splashy-download"></i></a><span class="deletetech" rel="'.$k.'_'.$mission['id'].'" id="'.$k.'_'.$mission['id'].'"> <i class="icon-adt_trash"></i></span></td></tr>';
			}
			$k++;
		}
		else:
		...
		foreach($exploded_file_paths as $row)
		{
			...
			if(file_exists($file_path) && !is_dir($file_path))
			{
					...
					if($fname=="")
						$fname = basename($row);
					$ofilename = pathinfo($file_path);
					$files .= '<tr><td width="30%">'.$fname.'</td><td width="35%">'.substr($ofilename['filename'],0,-3).".".$ofilename['extension'].'</td><td width="20%">'.formatSizeUnits(filesize($file_path)).'</td><td>SEO</td><td align="center" width="15%"><a href="/quote/download-document?type=seo_mission&mission_id='.$mission['id'].'&index='.$k.'"><i style="margin-right:5px" class="splashy-download"></i></a><span class="delete" rel="'.$k.'_'.$mission['id'].'" id="'.$k.'_'.$mission['id'].'"> <i class="icon-adt_trash"></i></span></td></tr>';
