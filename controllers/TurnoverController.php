...
class TurnoverController extends Ep_Controller_Action
{
	public function init()
	{
		...
		$this->_view->month_array_val=array("01"=> ucfirst(strftime("%b", mktime(0, 0, 0, '01'))), "02"=>ucfirst(strftime("%b", mktime(0, 0, 0, '02'))),
	...
	function realMonthAction()
	{
		...
		{
			$missionYear=explode('-',$realturnover['yearmonth']);
			$techtaskMission = $task_obj->getTechTaskMissions(array('contract_id'=>$realturnover['quotecontractid'],'m_status'=>'validated'));
			if($request['year']==$missionYear[0] && count($techtaskMission)==0)
			{
			$monthturnovers[$realturnover['user_id']]['other_info'] = $realturnover;
			...
			$monthturnovers[$realturnover['user_id']][$realturnover['yearmonth']] += $realturnover['missionturnover'];
				...
				if(count($techtaskMission)>0)
					{
						//echo "<pre>"; print_r($techtaskMission); exit;
					foreach($techtaskMission as $missionTastdetails)
						{
							if($request['year']==$missionTastdetails['validatedyear'])
							{
								$monthturnovers[$realturnover['user_id']]['other_info'] = $realturnover;
								$client_reallist[$realturnover['user_id']]=$realturnover['company_name'];

								if($missionTastdetails['oneshot']=='no')
									$totaltask=ceil($missionTastdetails['volume']/$missionTastdetails['volume_max']);
								else
									$totaltask=1;
								if($realturnover['package']=='team')
									$techtask['turnover'] = $missionTastdetails['task_turnover'];
									else
									$techtask['turnover'] = $missionTastdetails['task_turnover'];
									$monthturnovers[$realturnover['user_id']][$missionTastdetails['validatedyearmonth']] += $techtask['turnover'];
							}
						}
					}
				}
				if($request['client'])
            {
	            	if($request['client']!=$realturnover['user_id'])
            	unset($monthturnovers[$realturnover['user_id']]);	
            }
		}
		...
		{
			$techtaskMission = $task_obj->getTechTaskMissions(array('mission_id'=>$client_list['identifier'],'m_status'=>'validated'));
			$missionYear=explode('-',$client_list['yearmonth']);
			if($request['year']==$missionYear[0] && count($techtaskMission)==0)
			{
			$client_reallist[$client_list['user_id']]=$client_list['company_name'];
			if($client_list['currency']=='euro')
						...
						foreach($techtaskMission as $missionTastdetails)
						{
							if($request['year']==$missionTastdetails['validatedyear'])
							{
								...
								if($realturnover['package']=='team')
								 ...
								 if($client_list['currency']=='euro')
									{
									$realtotalturnovereuro += $techtask['turnover'];
								
									}
									else
									{
									$realtotalturnoverpound += $techtask['turnover'];
	...
	function realMonthClientFocusAction()
	{
		...
		if($request['client']&&$request['year'])
		{
			...
			foreach($contracts as $contract)
			{
				...
				{
					if($tech_mission['contractmissionid']!="")
					{
					$techtaskMission = $task_obj->getTechTaskMissions(array('cmid'=>$tech_mission['contractmissionid'],'m_status'=>'validated'));
					if($year==$tech_mission['validatedyear'] && count($techtaskMission)==0)
					{
						...
						//	echo "<pre>"; print_r($techtaskMission); 
					if(count($techtaskMission)>0)
					{
						foreach($techtaskMission as $missionTastdetails)
						{
							if($year==$missionTastdetails['validatedyear'])
							{
								if($tech_mission['oneshot']=='no')
									$totaltask=ceil($tech_mission['volume']/$tech_mission['volume_max']);
								else
									$totaltask=1;
								if($tech_mission['package']=='team')
									$techtask['turnover'] = $missionTastdetails['task_turnover'];
									else
									$techtask['turnover'] = $missionTastdetails['task_turnover'];
								$contractturnover[$contract['quotecontractid']][$tech_mission['identifier']][$missionTastdetails['validatedyearmonth']]  += $techtask['turnover'];
								$contractturnover[$contract['quotecontractid']]['contract_details'][$missionTastdetails['validatedyearmonth']] += $techtask['turnover'];
								$canvas_real[$missionTastdetails['validatedyearmonth']] +=  $techtask['turnover'];
								$contractturnover[$contract['quotecontractid']]['contract_details']['realturnover'] +=  $techtask['turnover'];
							}
						}					
					}
					}
			...
			//contract Mission details
				...
				$clienttrunarray[$clientloop['quotecontractid']][$splitbyclient['mission_id']][$splitbyclient['month_year'].'-from_followup']=$splitbyclient['from_followup'];
					...
					if(!$contractMissoinType)
					{
							...
							$searchParameters['include_final']='yes';
							$searchParameters['assigned_to']=$assigned_toval;
							$techMissoinType=$turnover_obj->getTechMissionDetails($searchParameters);
					...
					$clientloop['expected_end_date']=date('Y-m-d', strtotime($clientloop['expected_launch_date']. ' + '.$techMissoinType[0]['delivery_time'].' days'));
							if(date("mY",strtotime($clientloop['expected_launch_date'])) == date("mY",strtotime($clientloop['expected_end_date'])))
							{
									...
									$clienttrunarray[$clientloop['quotecontractid']][$splitbyclient['mission_id']]['cost-'.$splitbyclient['month_year']]=(((100-$techMissoinType[0]['margin_percentage'])/100)*$clienttrunarray[$clientloop['quotecontractid']][$splitbyclient['mission_id']][$splitbyclient['month_year']]);//$techMissoinType[0]['margin_percentage']*$techMissoinType[0]['volume'];
							}
							else
							{
		...
		foreach($clientcontractlist as $clientloop)
		{
			...
			{ //split turnover for contract 
				...
				if($year_montharray[0]==$default_year)
				{
					...
					if(!$contractMissoinType)
					{
								$searchParameters['contract_id']= $clientloop['quotecontractid'];
								$searchParameters['include_final']='yes';
								$searchParameters['identifier']= $splitbyclient['mission_id'];
								$techMissoinType=$contract_obj->getTechMissionDetails($searchParameters);
						$clientloop['expected_end_date']=date('Y-m-d', strtotime($clientloop['expected_launch_date']. ' + '.$techMissoinType[0]['delivery_time'].' days'));
						$clienttrunarray[$clientloop['quotecontractid']][$splitbyclient['mission_id']]['mission_type']=$techMissoinType[0]['title'];
								...
								if($techMissoinType[0]['contractmissionid']!='')
								{
							if(date("mY",strtotime($clientloop['expected_launch_date'])) == date("mY",strtotime($clientloop['expected_end_date'])))
							{
									$clienttrunarray[$clientloop['quotecontractid']][$splitbyclient['mission_id']]['cost'][$splitbyclient['month_year']]=(((100-$techMissoinType[0]['margin_percentage'])/100)*$clienttrunarray[$clientloop['quotecontractid']][$splitbyclient['mission_id']][$splitbyclient['month_year']]);//($techMissoinType[0]['cost']*$techMissoinType[0]['volume']);//*($techMissoinType[0]['delivery_time']);
			...
			foreach($cmissiondetails as $contract_mission)
			{
				...
				else
				{
					...
					{
						if($contract_mission['external_mission']=='yes')
						{
							$clienttrunarray[$clientloop['quotecontractid']]['realcostturnover'] += $contract_mission['freelance_cost'];	
							$clienttrunarray[$clientloop['quotecontractid']][$contract_mission['qmid']]['realcost_'.$contract_mission['validatedyearmonth']]  = $contract_mission['freelance_cost'];
						}
						else
						{
							$clienttrunarray[$clientloop['quotecontractid']]['realcostturnover'] += $contract_mission['internal_cost'];	
						$clienttrunarray[$clientloop['quotecontractid']][$contract_mission['qmid']]['realcost_'.$contract_mission['validatedyearmonth']]  = $contract_mission['internal_cost'];
						}
			...
			{
				$techtaskMission = $task_obj->getTechTaskMissions(array('cmid'=>$tech_mission['contractmissionid'],'m_status'=>'validated'));
				if($default_year==$tech_mission['validatedyear'] && $default_month==$tech_mission['validatedmonth'] && count($techtaskMission)==0)
				{
					if($tech_mission['external_mission']=='yes')
						$realcost1=$tech_mission['freelance_cost'];
					else
						$realcost1=($tech_mission['cost']*$tech_mission['volume']);
					...
					$clienttrunarray[$clientloop['quotecontractid']]['realcost_'.$tech_mission['validatedyearmonth']] +=$realcost1;
					$clienttrunarray[$clientloop['quotecontractid']]['realcostturnover'] += $realcost1;
					$clienttrunarray[$clientloop['quotecontractid']][$tech_mission['identifier']]['realcost_'.$tech_mission['validatedyearmonth']] +=$realcost1;
					//echo "<pre>".$tech_mission['turnover'].' turnover'.$realcost1;
				}
				else{
					if(count($techtaskMission)>0)
					{
						...
						foreach($techtaskMission as $missionTastdetails)
						{
							if($default_year==$missionTastdetails['validatedyear'])
							{
								if($tech_mission['oneshot']=='no')
								{
									$totaltask=ceil($tech_mission['volume']/$tech_mission['volume_max']);
								...
								if($tech_mission['package']=='team')
										$techtask['turnover']=$missionTastdetails['task_turnover'];//	$techtask['turnover'] = (($tech_mission['turnover']+$tech_mission['team_fee']*$tech_mission['team_packs'])/$totaltask);
									else
									$techtask['turnover'] = $missionTastdetails['task_turnover'];
								$realcost=$missionTastdetails['task_cost'];
								$clienttrunarray[$clientloop['quotecontractid']][$tech_mission['identifier']]['realcost_'.$missionTastdetails['validatedyearmonth']] +=$realcost;
								$clienttrunarray[$clientloop['quotecontractid']][$tech_mission['identifier']]['real_'.$missionTastdetails['validatedyearmonth']] += $techtask['turnover'];
								$clienttrunarray[$clientloop['quotecontractid']]['real_'.$missionTastdetails['validatedyearmonth']] += $techtask['turnover'];
								$clienttrunarray[$clientloop['quotecontractid']]['realcost_'.$missionTastdetails['validatedyearmonth']] +=$realcost;
								$clienttrunarray[$clientloop['quotecontractid']]['realturnover'] += $techtask['turnover'];
								$clienttrunarray[$clientloop['quotecontractid']]['realcostturnover'] += $realcost;
								//echo "<pre>".$techtask['turnover'].' turnover'.$realcost;
							}
						}					
				}
				}
