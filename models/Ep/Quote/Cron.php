...
//echo $query;   exit;
	...
	function getOnlySalesQuoteMissions()
	{
		$query = "SELECT CONCAT(up.first_name,' ',up.last_name) as sales_owner,q.sales_suggested_currency as currency ,q.sales_delivery_time,qc.contractname,qc.quotecontractid,qm.* FROM QuoteMissions  qm
				...
				WHERE q.client_id NOT IN ('144041410155143','143617233238522') AND q.sales_review='signed' AND qm.include_final='yes' AND qm.product NOT IN ('smo_audit','seo_audit','content_strategy') AND qm.identifier NOT IN (Select quote_mission_id FROM ProdMissions)
