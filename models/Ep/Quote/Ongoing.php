 ...
 */
	class Ep_Quote_Ongoing extends Ep_Db_Identifier
	{
		function getOngoingAODetails($searchParameters=NULL,$limit=NULL)
		{
			...
			$ongoingQuery="SELECT IF(c.company_name!='',c.company_name,u.email) as client,
						   ...
						   d.*,q.sales_suggested_currency,qm.product,qm.product_type,qm.product_type_other,qm.language_source,qm.language_dest, 	(SELECT max(proofread_end) FROM Article  WHERE delivery_id=d.id) as proofread_end,
	...
	{
		$query = "select a.*,d.AOtype,d.user_id,d.correction_type,d.contract_mission_id,d.correction as correctionao,cm.assigned_to,(cm.writing) as writing,(cm.proofreading) as proofreading,d.correction_file,d.id as did,d.user_id as clientid,(SELECT count(id) FROM Participation WHERE article_id='".$artId."' AND cycle=0) as totalpart,(SELECT count(id) FROM CorrectorParticipation WHERE article_id='".$artId."' AND cycle=0) as totalcorrectpart,(SELECT count(id) FROM Participation WHERE article_id='".$artId."' AND cycle=0 AND (status='bid_premium' OR status='bid_temp' OR status='bid_refused' OR status='bid_refused_temp')) as unselectedwriters,(SELECT count(id) FROM CorrectorParticipation WHERE article_id='".$artId."' AND cycle=0 AND (status='bid_corrector' OR status='bid_temp' OR status='bid_refused' OR status='bid_refused_temp')) as unselectedcorrectors,cm.conversion           
	...
	function getExtract2()
	{
		...
		WHERE p.status = 'published' AND d.missiontest = 'no' AND DATE(p.updated_at) >= '2015-05-01' AND DATE(p.updated_at) <= '2015-05-31' AND cm.type_id = qm.identifier
