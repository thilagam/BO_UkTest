...
class Ep_Quote_Turnover extends Ep_Db_Identifier
{
	function getRealTurnovers($search = NULL)
	{
		...
		$join
		WHERE d.user_id NOT IN ('144041410155143','143617233238522') AND
		d.missiontest='no' AND p.status='published' AND qc.status NOT IN('sales')
		$where
	...
	function getRealSeoMission($search=NULL)
	{
		...
		$query = "SELECT SUM(qm.turnover) as 			missionturnover,c.user_id,c.company_name,c.client_code,DATE_FORMAT(cm.validated_at, '%Y-%m' ) as yearmonth,q.sales_suggested_currency as currency
				  ...
				  WHERE q.client_id NOT IN ('144041410155143','143617233238522') AND cm.type='seo' 
				  $where 
	...
	function getRealTechMission($search=NULL)
	{
		...
		$query = "SELECT SUM(tm.turnover) as missionturnover,tm.*,c.user_id,c.company_name,c.client_code,DATE_FORMAT(cm.validated_at, '%Y-%m' ) as yearmonth,cm.currency,q.sales_suggested_currency as currency,qc.quotecontractid
				  FROM ContractMissions cm 
				  JOIN TechMissions tm ON tm.identifier = cm.type_id 
				  JOIN QuoteContracts qc ON qc.quotecontractid = cm.contract_id 
				  JOIN Quotes q ON q.identifier = qc.quoteid 
				  JOIN Client c ON c.user_id = q.client_id 
				  WHERE q.client_id NOT IN ('144041410155143','143617233238522') AND cm.type='tech' 
				  $where
				  GROUP BY MONTH(cm.validated_at),c.user_id";
		if(($res=$this->getQuery($query,true))!=NULL)
			return $res;
	...
	function getProdSeoMissions($search=NULL,$type="")
	{
		...
		cm.freeze_start_date,cm.freeze_end_date,DATE_FORMAT( cm.freeze_start_date, '%Y' ) as year_freeze_start_date,DATE_FORMAT( cm.freeze_end_date, '%Y' ) as year_freeze_end_date,cm.assigned_to,assigned_name.first_name,assigned_name.last_name
			...
			WHERE q.client_id NOT IN ('144041410155143','143617233238522') AND qm.include_final='yes' $where
	...
	//get Client Contract
	  ...
	  function getSplitTurnovers($cid)
		{
			$query = "SELECT * FROM ContractSplitValues WHERE contract_id='".$cid."' and mission_id !='' ORDER BY type,month_year";
			if(($result = $this->getQuery($query,true))!=NULL)
				return $result;
			else
				return false;
