...
class Ep_Quote_Recruitment extends Ep_Db_Identifier
{
	...
	{
		$allDetailsQuery="SELECT d.title as recruitment_title,d.publishtime,d.published_at,d.recruitment_file_name,d.link_quiz,d.quiz,d.quiz_marks,d.quiz_duration,					qm.*,qm.identifier as mission_id,q.*,q.identifier as quote_identifier,qc.contractname,
							...
							d.published_at as recruitment_launch,d.total_article,d.missiontest,d.num_hire_writers,d.recruitment_status,(SELECT max(senior_time) FROM Article  WHERE delivery_id=d.id ) as max_senior_time,(SELECT max(junior_time) FROM Article  WHERE delivery_id=d.id ) as max_junior_time,(SELECT max(subjunior_time) FROM Article  WHERE delivery_id=d.id ) as max_subjunior_time,
							(SELECT max(jc_resubmission) FROM Article  WHERE delivery_id=d.id ) as max_jc_resubmission,(SELECT max(jc0_resubmission) FROM Article  WHERE delivery_id=d.id ) as max_jc0_resubmission,(SELECT max(sc_resubmission) FROM Article  WHERE delivery_id=d.id ) as max_sc_resubmission,
							(SELECT max(proofread_end) FROM Article  WHERE delivery_id=d.id) as proofread_end,
							(SELECT max(participation_expires) FROM Article  WHERE delivery_id=d.id) as max_participation_time,d.resubmit_option,d.submit_option,d.file_name,d.correction_file,d.correction,
							(
