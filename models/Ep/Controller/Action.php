...
abstract class Ep_Controller_Action extends Zend_Controller_Action
{
	...
	public function init()
	{
        ...
        $incharge_info= $client_info_obj->getAllBoUsers();
        ...
        $this->_view->incharge_array=$aoIncharge;
	...
	public function getConfiguredval($constraint=NULL)
	{
		...
		//$conresult=$conf_obj->getConfiguration($constraint);
		 ...
		 if(count($conresult)>0 && $conresult!="NO" )
		 {
  ...
  /////////mails send in republish development////////////
    ...
    public function articleHistory($actionId, $actparams)
    {
        ...
        if($actparams['contributorId']) ///////get contributor name with userid
        {
            ...
            $contributor_name = '<a href="/user/contributor-edit?submenuId=ML2-SL7&tab=viewcontrib&userId="'.$actparams['contributorId'].'"><b>'.$userdetials[0]['first_name'].' '.$userdetials[0]['last_name'].'</b></a>';
        }
        if($actparams['correctorId']) ///////get corrector name with userid
        {
            $userdetials = $user_obj->getAllUsersDetails($actparams['correctorId']);
            $corrector_name = '<a href="/user/contributor-edit?submenuId=ML2-SL7&tab=viewcontrib&userId="'.$actparams['correctorId'].'"><b>'.$userdetials[0]['first_name'].' '.$userdetials[0]['last_name'].'</b></a>';
        }
		...
		if(!$this->adminLogin->userId)
			$this->adminLogin->userId='110823103540627';
		...
		if($_GET['debug'])
		{
			echo "<pre>";print_r($actparams);exit;
		}	
