...
class EP_Delivery_Article extends Ep_Db_Identifier
{
	...
	{	//print_r($step2);
		...
		{ 
			...
			$Aarray["language"] = $step2['language'][$p]; 
			...
			$Aarray["currency"] = $step2['currency']; 					
			$Aarray["status"] = "new";  
			...
			if($step2['correction']=="on")
			{
				...
				{
					$Aarray["correction_jc_submission"] = $step2['correction_jc_submission'][$p];
					$Aarray["correction_sc_submission"] = $step2['correction_sc_submission'][$p];
    ...
    //adding fetching of product(field) from article//
...
//exit;
	...
	public function getArticleDetailonly($article)
	{
		$query1 = "select * FROM Article WHERE id='".$article."'"; 
        $result1 = $this->getQuery($query1,true);
        return $result1;
    }
    /* added by naseer on 09-04-2015 */
    //function to fecth a token_is saved in articles
    public function getEbookersTokenIds($artId){
        $query = "SELECT `ebooker_tokenids` FROM ".$this->_name." WHERE `id` =".$artId;
        if(($result = $this->getQuery($query,true)) != NULL)
            return $result;
        else
            return "NO";
    }
    /* end of added by naseer on 09-04-2015 */
    /* *** added on 26.02.2016 *** */
    //function to check if it is a BNP mission or non BNP mission//
    public function checkIfBnpArticle($artId){
        $query = "SELECT (SELECT CM.`bnp_mission` FROM `ContractMissions` AS CM WHERE CM.`contractmissionid`=d.`contract_mission_id`) AS bnp_mission
                             FROM Participation p
                             INNER JOIN Article a ON a.id=p.article_id
                             INNER JOIN Delivery d ON a.delivery_id=d.id
                             WHERE a.id = ".$artId;
        if(($result = $this->getQuery($query,true)) != NULL)
            return $result[0]['bnp_mission'];
        else
            return "NO";
    }
}
