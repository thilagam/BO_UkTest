...
class Ep_Delivery_Pollquestion extends Ep_Db_Identifier
{
	...
	public function getQuestStats($poll,$smic="")
	{
		...
		$QuesQuery="SELECT pq.id,pq.title, pq.type, pq.option,count(pr.id) as partcount, max(CAST(pr.response AS UNSIGNED)) as max, min(CAST(pr.response AS UNSIGNED)) as min, avg(pr.response) as avg 
					FROM 
					Poll_question pq LEFT JOIN PollUserResponse pr ON pq.id=pr.question_id  LEFT JOIN Poll_Participation pp ON pp.user_id=pr.user_id AND pp.poll_id=pr.poll_id
					WHERE ".$smicCond."
