...
class Ep_User_UserPlus extends Ep_Db_Identifier
{
   ...
   UserPlus up ON up.user_id=u.identifier where 1=1 ORDER BY u.created_at DESC";
    ...
    {
        $profileQuery1 = "select count(identifier) AS never_participated FROM User WHERE type='contributor' and identifier not in (select distinct user_id from Participation)";
        $profileQuery2 = "select count(identifier) AS never_sent from User where type='contributor' and identifier IN (select distinct user_id from Participation where  status IN ('bid','bid_premium','bid_refused', 'bid_nonpremium', 'bid_temp', 'bid_refused_temp') )";
        ...
        $profileQuery3 = "select count(identifier) AS never_validated from User where type='contributor'
                          AND identifier  IN (select distinct user_id from Participation where  status NOT IN
                          ('bid','under_study','time_out','refused','ignored','disapproved','bid_premium','bid_refused','bid_nonpremium','on_hold','bid_temp','bid_refused_temp','bid_nonpremium_timeout','bid_premium_timeout','disapproved_temp','plag_exec','bid_pending'))";
        /*end of edited by naseer on 15-09-2015*/
        $profileQuery4 = "select count(identifier) AS once_validated from User where type='contributor' and identifier  IN (select distinct user_id from Participation where  status IN ('bid', 'under_study', 'disapproved', 'time_out', 'published', 'closed', 'on_hold'))";
        $profileQuery5 = "select count(identifier) AS total_contribs FROM User WHERE type='contributor'";
        $profileQuery6 = "select count(identifier) AS once_published FROM User WHERE type='contributor' and identifier  IN (select distinct user_id from Participation where  status IN ('published'))";
    ...
    //get the details related to participator and corrector//
    public function getPartUserinfo($participId){
        $query = "SELECT CONCAT( UP.`first_name` , ' ', UP.`last_name` ) AS Partname, AP.`id` , DATE_FORMAT(AP.`article_sent_at`,'%d-%m-%Y %h:%i') AS article_sent_at
            FROM `Participation` AS P
            LEFT JOIN `UserPlus` AS UP ON UP.`user_id` = P.`user_id`
            LEFT JOIN `ArticleProcess` AS AP ON AP.`participate_id` = P.`id`
            WHERE P.`user_id` = AP.`user_id`
            AND P.`id` = '" . $participId . "'
            ORDER BY AP.`article_sent_at` DESC
            LIMIT 0 , 1
        ";
        if(($result = $this->getQuery($query,true)) != NULL)
            return $result;
    ...
    //get the details related to participator and corrector//
    public function getCrtPartUserinfo($participId){
        $query = "SELECT CONCAT( UP.`first_name` , ' ', UP.`last_name` ) AS CrtPartname, AP.`id` , DATE_FORMAT(AP.`article_sent_at`,'%d-%m-%Y %h:%i') AS article_sent_at
            FROM `CorrectorParticipation` AS CP
            LEFT JOIN `Participation` AS P ON P.`id` = CP.`participate_id`
            LEFT JOIN `UserPlus` AS UP ON UP.`user_id` = CP.`corrector_id`
            LEFT JOIN `ArticleProcess` AS AP ON AP.`participate_id` = P.`id`
            WHERE CP.`corrector_id` = AP.`user_id`
            AND P.`id` = '" . $participId . "'
            ORDER BY AP.`article_sent_at` DESC
            LIMIT 0 , 1
        ";
