...
class EP_User_Contributor extends Ep_Db_Identifier
{
    ...
    {
        $ContribQuery = "select count(*) as count FROM User u INNER JOIN Contributor c ON u.identifier=c.user_id WHERE u.type='contributor' AND u.profile_type IN ('".$ptype."') AND u.status='active' AND u.blackstatus='no'" ;
...
//exit($languageCond);
        if($ptype1)
        {
            $ContribQuery = "SELECT u.identifier, c.category_more FROM User u INNER JOIN UserPlus up ON u.identifier = up.user_id INNER JOIN Contributor c ON c.user_id = u.identifier WHERE u.type = 'contributor' AND u.status = 'Active' AND u.blackstatus='no' {$languageCond} {$categoryCond} AND (u.profile_type IN ('".str_replace(",", "','", $ptype1)."')" ;
            if($ptype2)
        ...
        {
            $ContribQuery = "SELECT u.identifier, c.category_more FROM User u INNER JOIN UserPlus up ON u.identifier = up.user_id INNER JOIN Contributor c ON c.user_id = u.identifier WHERE u.type = 'contributor' AND u.status = 'Active' AND u.blackstatus='no' {$languageCond} {$categoryCond} AND u.profile_type IN ('senior','junior') AND u.profile_type2 IN ('".str_replace(",", "','", $ptype2) . "') ORDER BY u.updated_at DESC" ;
        }  
        else 
            $ContribQuery = "SELECT u.identifier, c.category_more FROM User u INNER JOIN UserPlus up ON u.identifier = up.user_id INNER JOIN Contributor c ON c.user_id = u.identifier WHERE u.type = 'contributor' AND u.status = 'Active' {$languageCond} {$categoryCond} ORDER BY u.updated_at DESC" ;
...
//exit($languageCond);
        ...
        else if($ptype2)
        {
            ...
            $ContribQuery = "SELECT u.identifier,up.first_name,up.last_name,u.email, u.profile_type,u.created_at, c.category_more,u.last_visit FROM User u LEFT JOIN UserPlus up ON u.identifier = up.user_id LEFT JOIN Contributor c ON c.user_id = u.identifier WHERE u.type = 'contributor' AND u.status = 'Active' {$languageCond} {$categoryCond} ORDER BY u.updated_at DESC" ;
    ...
    public function getContribsCount($params, $crt)
    {
	   ...
	   if($params['categ']!="")
        ...
        else if($ptype2)
        {
            ...
            $ContribQuery = "{$contribCountCond} u.type = 'contributor'{$crtCond} AND u.status = 'Active' {$languageCond} {$categoryCond} {$uidsCountCond}" ;
