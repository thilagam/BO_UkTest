...
class Ep_User_Jobs extends Ep_Db_Identifier
{
	...
	public function createJob($post)
	{
		...
		if($post['location_title']!="" && $post['location_desc']!="")
		{
			$jobarray['location_title']=$post['location_title'];
			$jobarray['location_desc']=$post['location_desc'];
		}
		else
		{
			$jobarray['location_title']=NULL;
			$jobarray['location_desc']=NULL;
		}
