...
abstract class Ep_Db_XmlDb
{
	...
	protected $_newPath = "/home/sites/site14/users/xmldb_editplace/translation";
	...
	function __construct($filePath, $rootName = '')
	{
		...
		{
			@fclose($fs);
			$mystring = file_get_contents($filePath);
			$checkEncoding = strstr($mystring, 'encoding="UTF-8"');
			if ($checkEncoding == false)
			{
				$string = substr_replace($mystring, ' encoding="UTF-8"', 19, 0);
				$fs = @fopen($filePath, "w+");
				@fwrite($fs, $string);
			}
			$this->_filePathName = $filePath;
