        ...
        {	
        	$ongoingQuery="SELECT (SELECT count(pa.id) 
							...
							(SELECT count(pa.id) 
								...
								WHERE pa.article_id=a.id AND pa.cycle=0 AND pa.status='bid_refused') as totalRefusedParticipations,
							...
							(SELECT cp.id FROM CorrectorParticipation cp 
								WHERE cp.status IN ('bid', 'under_study','disapproved_temp', 'disapproved','closed_temp', 'published','closed') AND cp.cycle=0 AND  cp.article_id=a.id AND cp.id=cp1.id LIMIT 1
