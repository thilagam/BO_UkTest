<?php /* Smarty version 2.6.19, created on 2016-01-29 14:30:34
         compiled from gebo/deliveryongoing/bulkarticlerepublish.phtml */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('modifier', 'utf8_encode', 'gebo/deliveryongoing/bulkarticlerepublish.phtml', 458, false),array('function', 'html_options', 'gebo/deliveryongoing/bulkarticlerepublish.phtml', 461, false),)), $this); ?>
<?php echo '
...
$(document).ready(function() {
   ...
   // loadEditors(\'allmailbody\');
	updatedBulkContribList2($("#aoId").val());
    $(\'textarea\').tinymce({
...
function changeAoType(aotype, aoId, artlist) ///when directly republish
{
    smoke.confirm("Do you really want to change type of AO",function(e){
        if (e)
        {
            var target_page = "/article-republish/changeaotype?&aoId="+aoId+"&aotype="+aotype;
            $.post(target_page, function(data){  //alert(data);
                var href="/article-republish/bulkrepublishpopup?nopart=no&close=no&aoId="+aoId+"&artlist="+artlist;
...
{
    var aoId = $("#aoId").val();
    var href="/article-republish/bulkrepublishpopup?nopart=no&close=no&aoId="+aoId;
    $("#republish").removeData(\'modal\');
    $(\'#republish .modal-body\').load(href);
    $("#republish").modal();
    $(".modal-backdrop:gt(0)").remove();
...
function saveRepublishPop() ///when directly republish
{
   ...
   // var price_max = $("#price_max").val();
    ...
    var target_page = "/article-republish/bulkrepublishpopup?artlist="+artlist+"&aoId="+aoId+"&save=save&aotype="+aotype+"&view_to="+checked+"&price_min="+price_min+"&price_max="+price_max+"&selectedcontribs="+selectedcontribs+"&fbcomments="+fbcomments+"&parttime="+parttime+"&parttime_option="+parttime_option+
    ...
    $.post(target_page, function(data){  //alert(data);
        ...
        else      /////////when republish pop up generated in selection profile related pages
        {
            smoke.confirm("L\\\'article sera remis en ligne",function(e){
                if (e)
                {
                    window.location.href = "/article-republish/bulkpublisharticlefo?aoId="+aoId+"&artlist="+artlist+"&aoType="+aotype+"&sendtofo=yes&sendrefusalmail="+sendrefusalmail+"&scobject="+scobject+"" +
                ...
                {
                    window.location.href = "/article-republish/bulkpublisharticlefo?aoId="+aoId+"artlist="+artlist+"&aoType="+aotype+"&sendtofo=no&sendrefusalmail="+sendrefusalmail+"&refusalmailcontent="+escape(refusalmessage);
                }
            });
        }
    });
...
function changeMailByContribType()
{
    ...
    var target_page = "/article-republish/changepublicmail?part_time="+extend_time+"&parttime_option="+parttime_option+"&artId="+artid+"&contribtype="+result;
...
function fngetbulkmailcontent()
{
    ...
    var parttime_option = $("#parttime_option").val();
    var extend_time = $("#participation_time").val();
    var target_page = "/article-republish/getbulkdynamicselectedcontribs?part_time="+extend_time+"&parttime_option="+parttime_option+"&scCount="+scCount+"&selectedcontribs="+selectedcontribs+"&aoId="+aoId+"&onlyscmail="+onlyscmail+"&checkedcontribs="+checkedcontribs;
    $.post(target_page, function(data){ // alert(data);
        if(aotype == "private"){
            var mailcontent = data.split("*");
            $(\'#scmailbody\').html(mailcontent[0]);
...
function publicAoContribList(artId)
{
    ...
    $.ajax({
        type: \'GET\',
        url: \'/article-republish/loadpublicaouserslist\',
...
" type="hidden"><div class="alert alert-success"><strong>Renvoyer l'article aux personnes ayant déjà participé sans être sélectionnées</strong> <span id="mailsenduserscount"> (<?php echo $this->_tpl_vars['createmailtobesentcount']; ?>
 contributeurs pourront participer)</span> </div>
...
?>
        ...
        <label class="control-label"> <?php echo $this->_tpl_vars['price_item']['arttitle']; ?>
:</label>
        <div class="controls form-inline">
    ...
    <div class="control-group formSep" id="privateaoset">
        ...
        <div class="controls form-inline">
            <label class="uni-checkbox">
                <input type="checkbox" id="contribtype1" name="contribtype[]"  value="senior" <?php if ($this->_tpl_vars['sc'] == 'yes'): ?> checked <?php endif; ?> class="uni_style" onClick="updatedBulkContribList2('<?php echo $this->_tpl_vars['aoId']; ?>
');" />SC <b>(<?php echo $this->_tpl_vars['sc_count']; ?>
)</b> </label>
            <label class="uni-checkbox">
                <input type="checkbox" id="contribtype2" name="contribtype[]"  value="junior" <?php if ($this->_tpl_vars['jc'] == 'yes'): ?> checked <?php endif; ?> class="uni_style" onClick="updatedBulkContribList2('<?php echo $this->_tpl_vars['aoId']; ?>
');" />JC  <b>(<?php echo $this->_tpl_vars['jc_count']; ?>
)</b></label>
            
			<?php if ($this->_tpl_vars['artdeldetails'][0]['product'] != 'translation'): ?>
			<label class="uni-checkbox">
                <input type="checkbox"  id="contribtype3" name="contribtype[]"  value="sub-junior" <?php if ($this->_tpl_vars['jc0'] == 'yes'): ?> checked <?php endif; ?> class="uni_style" onClick="updatedBulkContribList2('<?php echo $this->_tpl_vars['aoId']; ?>
');" />SUB JUNIOR <b>(<?php echo $this->_tpl_vars['jc0_count']; ?>
        ...
        <div class="controls form-inline">
            <select name="contrib_lang[]" id="contrib_lang" onChange="updatedBulkContribList2('<?php echo $this->_tpl_vars['aoId']; ?>
');" data-placeholder="select language" multiple="multiple" style="width:250px;display:none">
...
" <?php if ($this->_tpl_vars['langkey'] == $this->_tpl_vars['language']): ?> selected <?php endif; ?>><?php echo ((is_array($_tmp=$this->_tpl_vars['langitem'])) ? $this->_run_mod_handler('utf8_encode', true, $_tmp) : smarty_modifier_utf8_encode($_tmp)); ?>
</option>
                <?php endforeach; endif; unset($_from); ?>
            </select>
            <?php echo smarty_function_html_options(array('name' => 'contrib_cat','id' => 'contrib_cat','options' => ((is_array($_tmp=$this->_tpl_vars['Contrib_cats'])) ? $this->_run_mod_handler('utf8_encode', true, $_tmp) : smarty_modifier_utf8_encode($_tmp)),'selected' => $this->_tpl_vars['contrib_cat'],'style' => "width:250px;display:none",'onChange' => "updatedBulkContribList2(".($this->_tpl_vars['aoId']).")"), $this);?>
...
');" value="sub-junior" <?php if ($this->_tpl_vars['jc0'] == 'yes'): ?> checked <?php endif; ?> class="uni_style"  />SUB JUNIOR <b>(<?php echo $this->_tpl_vars['jc0_count']; ?>
			...
			<?php endif; ?>
            <div >  <select name="contrib_lang[]" id="contrib_lang" style="display:none" data-placeholder="select language" multiple="multiple" style="width:250px;padding-left: 50px;top:50px;position: relative;">
...
" type="hidden"/>
        <td colspan="5" class="alert alert-danger">
            <div class="span4 pull-right">
                <label class="uni-checkbox" style="float: left;"><input id="anouncebymail" name="anouncebymail" type="checkbox" class="uni_style " />Warn by email</label>
                <?php if ($this->_tpl_vars['aotype'] == 'public'): ?>
                <label class="uni-checkbox" style="float: left;"><input id="fbpost" name="fbpost" type="checkbox" class="uni_style " value="yes" onclick="showFbPost();"/>Post on FB and Twitter
                </label>
                <?php endif; ?>
