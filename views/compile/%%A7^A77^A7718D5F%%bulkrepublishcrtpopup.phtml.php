<?php /* Smarty version 2.6.19, created on 2014-08-26 16:14:45
         compiled from gebo/republish/bulkrepublishcrtpopup.phtml */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('modifier', 'utf8_encode', 'gebo/republish/bulkrepublishcrtpopup.phtml', 232, false),)), $this); ?>
<?php echo '
...
function saveCorrectorRepublishPop() ///when directly republish
{
   // var artId = $("#artId").val();
    ...
    var target_page = "/correction/bulkrepublishcorrectorpopup?aoId="+aoId+"&artlist="+artlist+"&save=save&parttime_option="+parttime_option+"&parttime="+parttime+"&suboptparttime="+suboptparttime+
                    ...
                    smoke.confirm("ANNOUNCE BY EMAIL",function(e){
                        if (e)
                        {
                            if(closerepublish == \'yes\')
                                window.location.href = "/correction/bulkpublishcrtarticlefo?aoId="+aoId+"&artlist="+artlist+"&sendmail=yes&sendrefusalmail=yes&refusalmailcontent="+refusalmessage+"&republishmail="+message+"&republishsubject="+object;
                            else
                                window.location.href = "/correction/bulkpublishcrtarticlefo?aoId="+aoId+"&artlist="+artlist+"&sendmail=yes&republishmail="+message+"&republishsubject="+object;
                        }
                        else
                        {
                            if(closerepublish == \'yes\')
